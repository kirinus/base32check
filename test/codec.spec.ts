...
import { Base32Variant, decode, encode } from '../src/codec';

describe('base32 codecs', () => {
  const rfc4648TestVectors = [
    ['', ''],
    ['66', 'MY======'],
    ['666f', 'MZXQ===='],
    ['666f6f', 'MZXW6==='],
  ...
  describe('decode', () => {
    rfc4648TestVectors.forEach(([output, payload]) =>
      test('RFC4648 - Test vectors', () => {
        expect(
          decode(payload, { variant: Base32Variant.Rfc4648 }),
        ).toStrictEqual(hexToArrayBuffer(output));
        expect(
          decode(payload, { variant: Base32Variant.Rfc3548 }),
        ).toStrictEqual(hexToArrayBuffer(output));
      }),
    );

    rfc4648HexTestVectors.forEach(([output, payload]) =>
      test('RFC4648 - Hex test vectors', () => {
        expect(
          decode(payload, { variant: Base32Variant.Rfc4648Hex }),
        ).toStrictEqual(hexToArrayBuffer(output));
      }),
    );

    rfc4648RandomData.forEach(([output, payload]) =>
      test('RFC4648 - Random data', () => {
        expect(
          decode(payload, { variant: Base32Variant.Rfc4648 }),
        ).toStrictEqual(hexToArrayBuffer(output));
        expect(
    ...
    ].forEach(([output, payload]) =>
      test('Crockford - Small samples', () => {
        expect(decode(payload)).toStrictEqual(hexToArrayBuffer(output));
      }),
    );
  });

  describe('encode', () => {
    rfc4648TestVectors.forEach(([payload, output]) =>
      test('RFC4648 - Test vectors', () => {
        expect(
          encode(hexToArrayBuffer(payload), { variant: Base32Variant.Rfc4648 }),
        ).toStrictEqual(output);
        expect(
          encode(hexToArrayBuffer(payload), {
            variant: Base32Variant.Rfc3548,
            padding: false,
          }),
        ).toStrictEqual(output.replace(/=/g, ''));
      }),
    );

    rfc4648HexTestVectors.forEach(([payload, output]) =>
      test('RFC4648 - Hex test vectors', () => {
        expect(
          encode(hexToArrayBuffer(payload), {
            variant: Base32Variant.Rfc4648Hex,
          }),
        ).toStrictEqual(output);
      }),
    );

    rfc4648RandomData.forEach(([payload, output]) =>
      test('RFC4648 - Random data', () => {
        expect(
          encode(hexToArrayBuffer(payload), { variant: Base32Variant.Rfc4648 }),
        ).toStrictEqual(output);
        expect(
          encode(hexToArrayBuffer(payload), { variant: Base32Variant.Rfc3548 }),
        ).toStrictEqual(output);
      }),
    );
    ...
    [
      ...
      ['6c696e7573', 'DHMPWXBK'],
      ['666f6f626172', 'CSQPYRK1E8'],
    ].forEach(([payload, output]) =>
      test('Crockford - Small samples', () => {
        expect(encode(hexToArrayBuffer(payload))).toStrictEqual(output);
      }),
    );

    describe('Payload Types', () => {
      const payload = [1, 2, 3, 4];
      const output = '0410610';

      test('Int8Array', () => {
        expect(encode(new Int8Array(payload))).toStrictEqual(output);
      });

      test('Uint8Array', () => {
        expect(encode(new Uint8Array(payload))).toStrictEqual(output);
      });

      test('Uint8ClampedArray', () => {
        expect(encode(new Uint8ClampedArray(payload))).toStrictEqual(output);
      });
    });
  });
});
